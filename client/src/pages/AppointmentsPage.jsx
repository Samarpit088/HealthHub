...
export default function AppointmentsPage() {
    ...
    <div>
      ...
      {bookings.length === 0 ? (
        <div className="flex flex-col animate-fadeIn items-center justify-center mt-20">
          <div className="font-fredoka font-bold text-3xl text-[#06182B] mt-10">
            No appointments
          </div>
          <div
            className=" inline-flex gap-1  mt-2 bg-[#143146]  p-3 relative px-6 py-2 font-bold rounded-3xl overflow-hidden border border-[#C2D6E3]
  ...
  text-white transition-colors duration-700 hover:text-[#06182B] hover:border-[#143146] cursor-pointer"
          >
            <Link to={"/"} className="relative z-10 opacity-0 animate-fadeIn">
              Home
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 animate-fadeIn items-center text-md bg-[#F8F8F7] mx-45 rounded-4xl mt-20 p-15">
          {bookings.map((booking) => (
            <div className="">
              <div
                key={booking._id}
                className="bg-[white] rounded-4xl mx-50 w-[800px] h-[120px] pt-2 px-4 flex items-center justify-between"
              >
                <div>
                  <div className="bg-white rounded-full w-16 h-16 overflow-hidden flex items-center justify-center">
                    <img
                      src={`http://localhost:4000/DocsPhotos/${booking.doctor?.photo}`}
                ...
                <div className="flex flex-col">
                  ...
                  <p className="font-semibold text-[#06182B] text-sm ml-10">
                    (MBBS-MD)
                  </p>
                </div>
                <div className="ml-20 mb-5">
                  <p className="font-semibold text-[#06182B] text-xl">
                    {booking.doctor?.specializeIn}
                  </p>
                </div>
                <div className="flex flex-col ml-30">
                  <p className="font-fredoka text-[#06182B]">
                    <strong>Date:</strong>{" "}
                    {new Date(booking.date).toLocaleDateString()}
                  </p>
                  <p className="font-fredoka text-[#06182B]">
                    <strong>At:</strong> {booking.doctor?.fromTime}am
                  </p>
                </div>
                <div className="ml-4">
                  <button
